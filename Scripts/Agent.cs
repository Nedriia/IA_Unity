...
public class Agent : MonoBehaviour {
	...
	public Capteurs raycast;
	public float fl,f,fr;
	...
	public float force_gauche;
	public float force_droite;
	...
	void Start () {
		...
		raycast = gameObject.GetComponent<Capteurs> ();

		//l = raycast.dis_l;
		fl = raycast.distance_devant_gauche;
		f = raycast.distance_devant;
		fr = raycast.distance_devant;
		//r = raycast.dis_fr;
	...
	void Update () {
		...
		fl = raycast.distance_devant_gauche;
		f = raycast.distance_devant;
		fr = raycast.distance_devant_droite;
		...
		if (!hasFailed) {
            ...
            force_gauche = MAX_ROTATION * leftForce;
            force_droite = MAX_ROTATION * rightForce;
			
			headingAngle += (force_gauche - force_droite) * Time.deltaTime;
	...
	public float Normalise(float i){
		float depth = i / raycast.Taille_Lasers;
		return 1 - depth;
	}
...
public class NNet {
	...
	public void FromGenome(Genome genome, int numofInputs, int neuronsPerHidden, int numOfOutputs){
		...
		//Debug.Log ("fromgenome, outputlayer neurons#: " + outneurons.Count);
...
public class NLayer {
	...
	public void Evaluate(List<float> input, ref List<float> output){
		...
		for (int i=0; i< totalNeurons; i++) {
            ...
            //    new System.IO.StreamWriter(@"C:\Users\arthu\Desktop\test\reseau.txt", true))
            //{
            //    file.WriteLine(activation);
            //}
            //test reseau
